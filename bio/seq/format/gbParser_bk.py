...
class Gb_Feature(object):
    ...
    locPattern = re.compile(r'\s{5}(\w+)\s+(\d+)\.\.(\d+)\s?')
    ...
    def loadfile(self,file):
        ...
        for each in file:
            ...
            if tmp:#qualifier line
                ...
                continue
            tmp = Gb_Feature.locPattern.match(each)
            if not tmp:#the next line of qualifier
                self.qua[self.quakeys[-1]][-1] += each.strip()
                continue
            if tmp:#first line
                self.fea = tmp.group(1)
